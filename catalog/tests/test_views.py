...
class AuthorListViewTest(TestCase):
    ...
    # перевіряємо код статусу після отримання відповіді на симульований запит на конкретний URL
    def test_view_url_exists_at_desired_location(self): 
        resp = self.client.get('/catalog/authors/') 
        self.assertEqual(resp.status_code, 200)  
    # перевіряємо код статусу після отримання відповіді на симульований запит по імені URL       
    def test_view_url_accessible_by_name(self):
        resp = self.client.get(reverse('authors'))
        self.assertEqual(resp.status_code, 200)
    
    # перевіряємо, чи використано потрібний шаблон    
    ...
    # перевіряємо, чи передається у шаблон через змінну context атрибут 'is_paginated', переві-  
    # рити його значення і переконатись, що у шаблон передається атрибут 'author_list'=10  
...
class LoanedBookInstancesByUserListViewTest(TestCase):
    ...
    def setUp(self):
        ...
        genre_objects_for_book = Genre.objects.all()# створює <QuerySet [<Genre: Fantasyc>]>
 ...
 # щоб переконатись,що відображення перейде на сторінку логування – напевно, така поведінка за за-    
 # мовчуванням визначена у домішці LoginRequiredMixin, від якої наш клас-відображення наслідується
